...
import type { FileMetadata, FolderMetadata, TransferProgress, TransferMetadata } from '@/types/transfer'
import type { ChunkData } from '@/types/transfer'
import { readFileInChunks, getChunkSize, createFileMetadata, createFolderMetadata } from '@/lib/fileUtils'
  ...
  const sendMetadata = useCallback(
    (files: File[], folders?: FolderMetadata[]) => {
      if (!dataChannel || dataChannel.readyState !== 'open') {
        setTransferState((prev) => ({ ...prev, error: 'Data channel not ready' }))
        return
      }

      const fileMetadata: FileMetadata[] = files.map((file) => createFileMetadata(file))
      ...
      for (const file of files) {
        ...
        try {
          for await (const chunk of readFileInChunks(file, chunkSize)) {
            const isLastChunk = fileBytesTransferred + chunk.byteLength >= file.size
