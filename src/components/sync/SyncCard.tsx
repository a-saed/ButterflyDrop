...
export function SyncCard({
  ...
  const totalSize =
    state?.localSnapshot.reduce((sum, file) => sum + file.size, 0) || 0;
              ...
              <div className="mt-3 pt-3 border-t space-y-2 text-xs animate-in fade-in slide-in-from-top-2 duration-200">
                {state.pendingChanges.local.length > 0 && (
                  <div>
                    ...
                    <span className="font-medium">
                      {state.pendingChanges.local.length}
                    </span>
                  </div>
                )}
                {state.pendingChanges.remote.length > 0 && (
                  <div>
                    ...
                    <span className="font-medium">
                      {state.pendingChanges.remote.length}
                    </span>
                  </div>
                )}
                {state.pendingChanges.conflicts.length > 0 && (
                  <div>
                    ...
                    <span className="font-medium">
                      {state.pendingChanges.conflicts.length}
                    </span>
                  </div>
                )}
                {state.error && (
